...
void decode_exec_inst(chip8_ctx_t* ctx, uint16_t inst) {
	...
	switch (opcode) {
		...
		case OPCODE_8: {
			...
			} else if (N == SUBOPCODE_BINARY_OR) {
				ctx->gp_regs[X] = ctx->gp_regs[X] | ctx->gp_regs[Y];
			} else if (N == SUBOPCODE_BINARY_AND) {
				ctx->gp_regs[X] = ctx->gp_regs[X] & ctx->gp_regs[Y];
			} else if (N == SUBOPCODE_LOGICAL_XOR) {
				ctx->gp_regs[X] = ctx->gp_regs[X] ^ ctx->gp_regs[Y];
			} else if (N == SUBOPCODE_ADD) {
				ctx->gp_regs[X] = ctx->gp_regs[X] + ctx->gp_regs[Y];
			} else if (N == SUBOPCODE_SUB) {
				ctx->gp_regs[X] = ctx->gp_regs[X] - ctx->gp_regs[Y];
			} else if (N == SUBOPCODE_SHIFT_RIGHT) {
				ctx->gp_regs[X] = ctx->gp_regs[Y];
				ctx->gp_regs[GP_VF] = ctx->gp_regs[X] & 1;
				ctx->gp_regs[X] = ctx->gp_regs[X] >> 1;
			} else if (N == SUBOPCODE_SUB_SWAP) {
				ctx->gp_regs[X] = ctx->gp_regs[Y] - ctx->gp_regs[X];
			} else if (N == SUBOPCODE_SHIFT_LEFT) {
				ctx->gp_regs[X] = ctx->gp_regs[Y];
